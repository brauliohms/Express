...
test("Can be apply mask in CNPJ string", () => {
  ...
  expect(Formatter.formatCNPJToDisplay(cnpj13)).toStrictEqual("01.123.123/0001")
...
test("Can be apply mask in CPF string", () => {
  ...
  expect(Formatter.formatCNPJToDisplay(cpfFull)).toStrictEqual("012.345.678-90")
...
test("Can be convert to Slug Lowercase", () => {
  const case1 = "São Carlos do Ivaí"
  const case2 = " São Carlos do Ivaí "
  expect(Formatter.textToSlugLower(case1)).toStrictEqual("sao carlos do ivai")
  expect(Formatter.textToSlugLower(case2)).toStrictEqual("sao carlos do ivai")
})
